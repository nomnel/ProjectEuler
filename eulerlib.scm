...
(define (primes n)
  (if (< n 2) '()
      (let1 h (make-hash-table)
        (hash-table-put! h 2 #t)
        (let loop ((c 3))
          (cond ((< n c)
                 (sort (hash-table-fold h (^(k v p) (if v (cons k p) p)) '())))
                (else (unless (hash-table-exists? h c)
                        (hash-table-put! h c #t)
                        (dolist (d (range (* 2 c) (+ n 1) c))
                          (hash-table-put! h d #f)))
                      (loop (+ c 2))))))))
